# Given an array of integers nums and an integer target, return indices i and j such that nums[i] + nums[j] == target and i != j.
# You may assume that each input would have exactly one pair of indices i and j that satisfy the condition.
# Return the answer with the smaller index first.

# Example 1: Input: nums = [3, 4, 5, 6], target = 7 Output: [0,1] Explanation: nums[0] + nums[1] == 7, so we return [0, 1].
# Example 2: Input: nums = [4, 5, 6], target = 10, Output: [0,2] Explanation: nums[0] + nums[2] == 10, so we return [0, 2].

# Constraints:
# 2 <= nums.length <= 1000
# -10,000,000 <= nums[i] <= 10,000,000
# -10,000,000 <= target <= 10,000,000

class Solution:
  def twoSum(self, nums: List[int], target: int) -> List[int]:
      prevMap = {}  # val -> index

      for i, n in enumerate(nums):
          diff = target - n
          if diff in prevMap:
              return [prevMap[diff], i]
          prevMap[n] = i

# Time: O(n)
# Space: O(n)